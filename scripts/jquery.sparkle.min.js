...
extension:function(a,b){var d=c(this);a=c.Sparkle;d.findAndSelf(b.selector).findAndSelf(b.innerSelector).each(function(){var e=c(this),f=e.text().trim();b.empty.has(f)?e.addClass(b.emptyClass):e.addClass(b.notemptyClass)});return d}}});c.Bespin=new c.BalClass({"default":{content:null,bespin:{settings:{tabstop:4}}},rich:{bespin:{syntax:"html"}},simple:{}});c.fn.Bespin=function(a,b){var d=c.Bespin.getConfigWithDefault(a,b),e=c(this),f=e,g=e.attr("id")+"-bespin";if(e.is("textarea")){f=c('<div id="'+
g+'"/>').html(e.val()).css({height:e.css("height"),width:e.css("width")});f.insertAfter(e);e.hide()}window.onBespinLoad=function(){bespin.useBespin(g,d.bespin).then(function(h){var i=h.editor;e.is("textarea")&&e.parents("form:first").submit(function(){e.val(i.value)});if(d.content||d.content==="")i.value=d.content},function(h){throw Error("Launch failed: "+h);})};return e};c.Tinymce=new c.BalClass({"default":{script_url:"/scripts/tiny_mce/tiny_mce.js",theme:"advanced",plugins:"autoresize,safari,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template",
