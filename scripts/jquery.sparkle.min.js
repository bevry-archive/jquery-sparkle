...
a.fn.highlight||function(b){return a(this).effect("highlight",{},b||3E3)};a.fn.htmlAndSelf=a.fn.htmlAndSelf||function(){return a(this).attr("outerHTML")};a.fn.preventDefault=a.fn.preventDefault||function(){return a(this).click(function(b){b.preventDefault();return false})};a.fn.attemptTypeChangeTo=a.fn.attemptTypeChangeTo||function(b){var c=a(this),d=false,e=c.get(0),f=e.type;if(b===f)d=true;else if(c.is("input"))if(!a.browser.msie){e.type=b;if(e.type!==f)d=true}return d}})(jQuery);
...
(function(a){a(document.body).addClass("js");if(a.Sparkle)window.console.warn("$.Sparkle has already been defined...");else{a.SparkleClass=a.BalClass.clone({addExtension:function(b,c,d){var e=a.Sparkle,f={};switch(true){case Boolean(d||false):if(typeof b==="string"&&typeof d==="function"&&typeof c==="object"){f.extension=d;f.config=c;f.name=b}if(typeof b==="string"&&typeof c==="function"&&typeof d==="object"){f.extension=c;f.config=d;f.name=b}else throw Error("Sparkle.addExtension: Invalid Input");
break;case Boolean(c||false):if(typeof b==="string"&&typeof c==="function"){f.extension=c;f.name=b}else if(typeof b==="string"&&e.isExtension(c)){f=c;f.name=b}else throw Error("Sparkle.addExtension: Invalid Input");break;case Boolean(b||false):if(e.isExtension(b))f=b;else if(typeof b==="object"||typeof b==="array")a.each(b,function(g,j){e.addExtension(g,j)});else throw Error("Sparkle.addExtension: Invalid Input");break}f.config=f.config||{};f.extension=f.extension||{};e.addConfig(f.name,f);return true},
cycleExtensions:function(){var b=a.Sparkle,c=b.getExtensions();a.each(c,function(d,e){b.triggerExtension(e)});return true},hasExtension:function(b){return a.Sparkle.getExtension(b)!=="undefined"},isExtension:function(b){return Boolean(b.extension||false)},getExtensions:function(){var b=a.Sparkle,c=b.getConfig(),d={};a.each(c,function(e,f){if(b.isExtension(f))d[e]=f});return d},getExtension:function(b){var c=a.Sparkle,d=undefined;if(c.isExtension(b))d=b;else{b=c.getConfigWithDefault(b);if(c.isExtension(b))d=
b}return d},getExtensionConfig:function(b){return a.Sparkle.getExtension(b).config||{}},applyExtensionConfig:function(b,c){a.Sparkle.applyConfig(b,{config:c});return true},triggerExtension:function(b){var c=a.Sparkle;b=c.getExtension(b);var d=this instanceof jQuery?this:a("body");if(b)return b.extension.apply(d,[c,b.config,b]);return false},fn:function(b){var c=a.Sparkle,d=a(this);b?c.triggerExtension.apply(d,[b]):c.cycleExtensions.apply(d,[]);return d},built:function(){var b=this;a.fn.sparkle=function(c){return b.fn.apply(this,
[c])};a(function(){a(document.body).sparkle()});return true}});a.Sparkle=a.SparkleClass.create({config:{date:{config:{selector:".sparkle-date",datepickerOptions:{},demoText:"Date format must use the international standard: [year-month-day]. This due to other formats being ambigious eg. day/month/year or month/day/year.",demo:'<input type="text" class="sparkle-date" value="2010-08-05" />'},extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;if(typeof d.Datepicker===
"undefined"){window.console.warn("Datepicker not loaded. Did you forget to include it?");return false}d.Datepicker(c.datepickerOptions);return true}},time:{config:{selector:".sparkle-time",timepickerOptions:{},demoText:"Time format must be either [hour:minute:second] or [hour:minute], with hours being between 0-23.",demo:'<input type="text" class="sparkle-time" value="23:11" />'},extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;if(typeof d.Timepicker==="undefined"){window.console.warn("Timepicker not loaded. Did you forget to include it?");
return false}d.Timepicker(c.timepickerOptions);return true}},datetime:{config:{selector:".sparkle-datetime",datepickerOptions:{},timepickerOptions:{},demoText:"Date format must use the international standard: [year-month-day]. This due to other formats being ambigious eg. day/month/year or month/day/year.<br/>\t\t\t\t\t\t\tTime format must be either [hour:minute:second] or [hour:minute], with hours being between 0-23.",demo:'<input type="text" class="sparkle-datetime" value="2010-08-05 23:10:09" />'},
extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;if(typeof d.Datetimepicker==="undefined"){window.console.warn("Datetimepicker not loaded. Did you forget to include it?");return false}d.Datetimepicker({datepickerOptions:b.getExtensionConfig("date").datepickerOptions,timepickerOptions:b.getExtensionConfig("time").timepickerOptions});return true}},"hide-if-empty":{config:{selector:".sparkle-hide-if-empty:empty",demo:'<div class="sparkle-hide-if-empty" style="border:1px solid black"></div>\n<div class="sparkle-hide-if-empty" style="border:1px solid black">Hello World</div>'},
extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;d.hide();return true}},hide:{config:{selector:".sparkle-hide",demo:'<div class="sparkle-hide">Something to Hide when Sparkle has Loaded</div>'},extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;d.removeClass(c.selector.replace(".","")).hide();return true}},show:{config:{selector:".sparkle-show",demo:'<div class="sparkle-show" style="display:none;">Something to Show when Sparkle has Loaded</div>'},
extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;d.removeClass(c.selector.replace(".","")).show();return true}},subtle:{config:{selector:".sparkle-subtle",css:{},inSpeed:200,inCss:{opacity:1},outSpeed:400,outCss:{opacity:0.5},demo:'<div class="sparkle-subtle">This is some subtle text. (mouseover)</div>'},extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;var e={};a.extend(e,c.outCss,c.css);d.css(e).opacityFix().hover(function(){a(this).stop(true,
false).animate(c.inCss,c.inSpeed)},function(){a(this).stop(true,false).animate(c.outCss,c.outSpeed)});return true}},panelshower:{config:{selectorSwitch:".sparkle-panelshower-switch",selectorPanel:".sparkle-panelshower-panel",inSpeed:200,outSpeed:200,demo:""},extension:function(b,c){var d=a(this),e=d.findAndSelf(c.selectorSwitch);d=d.findAndSelf(c.selectorPanel);if(!e.length&&!d.length)return true;e.once("click",{clickEvent:function(){var f=a(this),g=f.siblings(c.selectorPanel).filter(":first"),j=
f.val();f.is(":checked,:selected")&&!(!j||j===0||j==="0"||j==="false"||j===false||j==="no"||j==="off")?g.fadeIn(c.inSpeed):g.fadeOut(c.outSpeed)}}.clickEvent);d.hide();return true}},autogrow:{config:{selector:"textarea.autogrow,textarea.autosize",demo:'<textarea class="autogrow">This textarea will autogrow with your input. - Only if jQuery Autogrow has been loaded.</textarea>'},extension:function(b,c){var d=a(this).findAndSelf(c.selector);if(!d.length)return true;if(typeof a.fn.autogrow==="undefined"){window.console.warn("autogrow not loaded. Did you forget to include it?");
return false}d.autogrow();return true}},gsfnwidget:{config:{selector:".gsfnwidget",demo:'<a class="gsfnwidget" href="#">This link will show a GetSatisfaction Widget onclick. - Only if GetSatisfaction has been loaded.</a>'},extension:function(b,c){var d={clickEvent:function(f){if(typeof GSFN_feedback_widget==="undefined"){window.console.warn("GSFN not loaded. Did you forget to include it?");return true}GSFN_feedback_widget.show();f.preventDefault();return false}},e=a(this).findAndSelf(c.selector);
if(!e.length)return true;e.once("click",d.clickEvent);return true}},hint:{config:{selector:".form-input-tip,.sparkle-hint,.sparkle-hint-has,:text[placeholder]",hasClass:"sparkle-hint-has",hintedClass:"sparkle-hint-hinted",demoText:"Simulates HTML5's <code>placeholder</code> attribute for non HTML5 browsers. Placeholder can be the <code>title</code> or <code>placeholder</code> attribute. Placeholder will not be sent with the form (unlike most other solutions). The <code>sparkle-hint</code> class is optional if you are using the <code>placeholder</code> attribute.",
