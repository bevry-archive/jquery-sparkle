...
Number.prototype.roundTo=String.prototype.roundTo=String.prototype.roundTo||function(){var a=String(parseInt(this,10));return a=parseInt(a.replace(/[1,2]$/,0).replace(/[3,4]$/,5),10)};String.prototype.trim=String.prototype.trim||function(){return this.replace(/^\s+|\s+$/g,"")};String.prototype.strip=String.prototype.strip||function(a,b){a=String(a);b||0||(a=a.replace(/([\[\]\(\)\^\$\.\?\|\/\\])/g,"\\$1"));var c=this.replace(eval("/^"+a+"+|"+a+"+$/g"),"");return String(c)};
String.prototype.stripLeft=String.prototype.stripLeft||function(a,b){a=String(a);b||0||(a=a.replace(/([\[\]\(\)\^\$\.\?\|\/\\])/g,"\\$1"));var c=this.replace(eval("/^"+a+"+/g"),"");return String(c)};String.prototype.stripRight=String.prototype.stripRight||function(a,b){a=String(a);b||0||(a=a.replace(/([\[\]\(\)\^\$\.\?\|\/\\])/g,"\\$1"));var c=this.replace(eval("/"+a+"+$/g"),"");return String(c)};String.prototype.toInt=String.prototype.toInt||function(){return parseInt(this,10)};
...
f.split(".");if(j.length===1)b[f]=e;else{var k="",n="";$.each(j,function(o,h){k+='["'+h.replace(/"/g,'\\"')+'"]';jsonCLOSUREGLOBAL=b;n="if ( typeof jsonCLOSUREGLOBAL"+k+' === "undefined" ) jsonCLOSUREGLOBAL'+k+" = {}";eval(n);b=jsonCLOSUREGLOBAL;delete jsonCLOSUREGLOBAL});jsonCLOSUREGLOBAL=b;valueCLOSUREGLOBAL=e;n="jsonCLOSUREGLOBAL"+k+" = valueCLOSUREGLOBAL";eval(n);b=jsonCLOSUREGLOBAL;delete jsonCLOSUREGLOBAL;delete valueCLOSUREGLOBAL}}}}}return b};
...
d.strengthCss.medium,d.strengthCss.high,d.strengthCss.empty].join(" "),j=a(d.password),k=a(d.confirm||null),n=a(d.username||null);c=function(){var o=j.val(),h=k.val(),i=n.val();h=o?o.passwordstrength(h,i):"empty";o=d.strengthCss[h];h=d.il8n[h];f.removeClass(g).addClass(o).html(h)};j.keyup(function(){var o=a(this);k.val("");o.val()!==""&&!e.data("shown")&&e.animate({height:"show",opacity:"show"},"slow").data("shown",true)});j.add(k).add(n).keyup(c);c();return b}})(jQuery);
...
m=i.getMonth()+1;if(typeof this.entriesByYearMonth[p+"-"+m]==="undefined")this.entriesByYearMonth[p+"-"+m]={};this.entriesByYearMonth[p+"-"+m][h.id]=h;if(m===11){i.setMonth(0);i.setYear(p+1)}else i.setMonth(m+1)}return this.entriesById[h.id]=h}};f.addEntries(e.calendarEntries);var g=function(h,i,l){var p=f.getEntriesByYearMonth(i,l);h=a(h).find("tbody td");var m=h.find("a");if(e.disableClick){h.unbind("click").removeAttr("onclick");m.removeAttr("href").css("cursor","default")}a.each(p,function(H,
r){var s=r.start.getMonth()+1,t=r.finish.getMonth()+1,u=r.start.getDate(),v=r.finish.getDate();u=s==l?m.filter(":contains("+u+"):first"):m.filter(":first");v=t==l?m.filter(":contains("+v+"):first"):m.filter(":last");s=s==l?m.index(u):0;t=t==l?m.index(v):m.length-1;var y=[];y=s==t?u:s==t-1?u.add(v):u.add(m.filter(":lt("+t+")").filter(":gt("+s+")")).add(v);y.addClass(e.dayEventClass).each(function(I,B){var w=a(B);w.text().trim();var q=w.data("dayEntriesIds");if(typeof q==="undefined")q=r.id;else{q=
...
h,i)},50);else{l=a.extend({},{year:h,month:i},e.ajaxPostData);a.ajax({url:e.ajaxEntriesUrl,method:"post",dataType:"json",data:l,success:function(m){m=m[e.ajaxEntriesVariable]||[];f.setCacheable(h,i,true);if(m.length===0)return true;f.addEntries(m);setTimeout(function(){g(p,h,i)},50);return true}})}else f.isYearMonthEmpty(h,i)||setTimeout(function(){g(p,h,i)},50);return true},k=false,n=function(h,i,l){if(!k){k=true;a(l).addClass(e.calendarClass);j(h,i,l)}},o=a.extend({},e.datepickerOptions,{onChangeMonthYear:function(h,
i,l){j(h,i,l);typeof e.datepickerOptions.onChangeMonthYear==="function"&&n(h,i,l)},beforeShow:function(h,i){datepickerShowed=true;typeof e.datepickerOptions.beforeShow==="function"&&e.datepickerOptions.beforeShow.apply(this,[h,i]);setTimeout(function(){n(i.drawYear,i.drawMonth+1,i)},1E3)}});d.datepicker(o);setTimeout(function(){var h=d.datepicker("getDate");n(h.getFullYear(),h.getMonth()+1,d)},2E3);return d},built:function(){var b=this;a.fn.EventCalendar=function(c,d){return b.fn.apply(this,[c,d])};
return true}})})(jQuery);
...
g+'">'+g.padLeft("0",2)+"</option>");var j=a('<select class="sparkle-time-minutes" />');for(f=0;f<=55;f+=5)j.append('<option value="'+f+'">'+f.padLeft("0",2)+"</option>");var k=a('<select class="sparkle-time-meridian" />');k.append("<option>am</option>");k.append("<option>pm</option>");g=d.val();var n=new Date;f="12";var o="0",h="am";if(g){n.setTimestr(g);f=n.getUTCHours();o=n.getUTCMinutes();if(f>12){f-=12;h="pm"}}k.insertAfter(d);j.insertAfter(d);e.insertAfter(d);if(f>12&&h=="pm")f-=12;e.val(f);
