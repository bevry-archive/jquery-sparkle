...
extension:function(a,b){var d=c(this);a=c.Sparkle;d.findAndSelf(b.selector).findAndSelf(b.innerSelector).each(function(){var e=c(this),f=e.text().trim();b.empty.has(f)?e.addClass(b.emptyClass):e.addClass(b.notemptyClass)});return d}}});c.Bespin=new c.BalClass({"default":{content:null,bespin:{settings:{tabstop:4}},toolbar:{fullscreen:true}},rich:{bespin:{syntax:"html"}},html:{bespin:{syntax:"html"}},plain:{toolbar:false}});c.Bespin.fn=function(a,b){var d=c.Bespin,e=d.getConfigWithDefault(a,b),f=this;
c(window).bind("onBespinLoad",function(){d.useBespin(f,e)});window.onBespinLoad=function(){c(window).trigger("onBespinLoad")};return this};c.Bespin.useBespin=function(a,b){var d=c.Bespin,e=c(a),f,g;if(e.is("textarea")){g=e.attr("id")+"-bespin";f=c('<div id="'+g+'"/>').html(e.val()).css({height:e.css("height"),width:e.css("width")});f.insertAfter(e);e.hide()}else{f=e;g=f.attr("id")}bespin.useBespin(g,b.bespin).then(function(h){d.postBespin(g,h,b)},function(h){throw Error("Bespin Launch Failed: "+h);
});return this};c.Bespin.postBespin=function(a,b,d){a=c("#"+a);var e=a.siblings("textarea"),f=b.editor;a.css("overflow","hidden");a.wrap('<div class="bespin-wrap" />');var g=a.parent();e.length&&e.parents("form:first").submit(function(){e.val(f.value)});if(d.content||d.content==="")f.value=d.content;if(d.toolbar){$toolbar=c('<div class="bespin-toolbar" />');$toolbar.insertBefore(a);if(d.toolbar.fullscreen){$fullscreen=c('<span class="bespin-toolbar-fullscreen">Fullscreen</span>');$fullscreen.appendTo($toolbar);
$fullscreen.click(function(){g.hasClass("bespin-fullscreen")?c("body").add(g).removeClass("bespin-fullscreen"):c("body").add(g).addClass("bespin-fullscreen");b.dimensionsChanged()})}}return this};c.fn.Bespin=function(a,b){return c.Bespin.fn.apply(this,[a,b])};c.Tinymce=new c.BalClass({"default":{script_url:"/scripts/tiny_mce/tiny_mce.js",theme:"advanced",plugins:"autoresize,safari,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template",
