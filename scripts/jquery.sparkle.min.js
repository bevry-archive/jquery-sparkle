...
a.fn.highlight||function(b){return a(this).effect("highlight",{},b||3E3)};a.fn.htmlAndSelf=a.fn.htmlAndSelf||function(){return a(this).attr("outerHTML")}})(jQuery);
...
d.strengthCss.medium,d.strengthCss.high,d.strengthCss.empty].join(" "),j=a(d.password),k=a(d.confirm||null),m=a(d.username||null);c=function(){var n=j.val(),h=k.val(),i=m.val();h=n?n.passwordstrength(h,i):"empty";n=d.strengthCss[h];h=d.il8n[h];f.removeClass(g).addClass(n).html(h)};j.keyup(function(){var n=a(this);k.val("");n.val()!==""&&!e.data("shown")&&e.animate({height:"show",opacity:"show"},"slow").data("shown",true)});j.add(k).add(m).keyup(c);c();return b}})(jQuery);
...
var f=a('<input type="text" class="sparkle-date"/>'),g=a('<span class="sparkle-datetime-sep"> @ </span>'),j=a('<input type="text" class="sparkle-time"/>'),k=e.val(),m=new Date,n="",h="";if(k){m.setDatetimestr(k);n=m.getDatestr();h=m.getTimestr()}j.insertAfter(e);g.insertAfter(e);f.insertAfter(e);f.val(n);j.val(h);f.add(j).change(function(){var i=f.val()+" "+j.val();e.val(i).trigger("change")});f.datepicker(d.datepickerOptions);j.timepicker(d.timepickerOptions);return e})},built:function(){var b=this;
...
if(isNaN(i)||i.toString()==="Invalid Date"){i=new Date;i.setDatetimestr(h)}return i},addEntries:function(h){var i=this;a.each(h,function(l,o){o.id=o.id||l;i.addEntry(o)});return true},addEntry:function(h){h.start=this.getDate(h.start);h.finish=this.getDate(h.finish);var i=this.getDate(h.start);i.setDate(1);i.setHours(0);i.setMinutes(0);i.setSeconds(0);i.setMilliseconds(0);var l=this.getDate(h.finish);l.setDate(2);l.setHours(0);l.setMinutes(0);l.setSeconds(0);for(l.setMilliseconds(0);i<l;){var o=i.getFullYear(),
p=i.getMonth()+1;if(typeof this.entriesByYearMonth[o+"-"+p]==="undefined")this.entriesByYearMonth[o+"-"+p]={};this.entriesByYearMonth[o+"-"+p][h.id]=h;if(p===11){i.setMonth(0);i.setYear(o+1)}else i.setMonth(p+1)}return this.entriesById[h.id]=h}};f.addEntries(e.calendarEntries);var g=function(h,i,l){var o=f.getEntriesByYearMonth(i,l),p=a(h).find("tbody td"),q=p.find("a");if(e.disableClick){p.unbind("click").removeAttr("onclick");q.removeAttr("href").css("cursor","default")}a.each(o,function(J,s){var t=
s.start.getMonth()+1,u=s.finish.getMonth()+1,v=s.start.getDate(),w=s.finish.getDate();v=t==l?q.filter(":contains("+v+"):first"):q.filter(":first");w=u==l?q.filter(":contains("+w+"):first"):q.filter(":last");t=t==l?q.index(v):0;u=u==l?q.index(w):q.length-1;var z=[];z=t==u?v:t==u-1?v.add(w):v.add(q.filter(":lt("+u+")").filter(":gt("+t+")")).add(w);z.addClass(e.dayEventClass).each(function(K,E){var x=a(E);x.text().trim();var r=x.data("dayEntriesIds");if(typeof r==="undefined")r=s.id;else{r=String(r).split(/,/g);
r.push(s.id);r=r.join(",")}x.data("dayEntriesIds",r);a.each(e.domEvents,function(A,F){x.unbind(A).bind(A,function(G){var y=a(this),B=y.text().trim();y=String(y.data("dayEntriesIds")).split(/,/g);var C=new Date;C.setDatestr(i+"-"+l+"-"+B);var D=[];a.each(y,function(L,H){var I=f.getEntryById(H);D.push(I)});F.apply(this,[G,{year:i,month:l,day:B,date:C,dayEntries:D,monthEntries:o,datepicker:h}]);return true})})})});return true},j=function(h,i,l){var o=l.dpDiv||l;if(typeof e.ajaxEntriesUrl==="string"&&
e.ajaxEntriesUrl.length)if(e.ajaxCache&&f.isCacheable(h,i)&&!f.isYearMonthEmpty(h,i))setTimeout(function(){g(o,h,i)},50);else{l=a.extend({},{year:h,month:i},e.ajaxPostData);a.ajax({url:e.ajaxEntriesUrl,method:"post",dataType:"json",data:l,success:function(p){p=p[e.ajaxEntriesVariable]||[];f.setCacheable(h,i,true);if(p.length===0)return true;f.addEntries(p);setTimeout(function(){g(o,h,i)},50);return true},error:function(){window.console.warn("$.EventCalendar.calendarChangeMonthYear.ajax.error:",[this,
arguments])}})}else f.isYearMonthEmpty(h,i)||setTimeout(function(){g(o,h,i)},50);return true},k=false,m=function(h,i,l){if(!k){k=true;a(l).addClass(e.calendarClass);j(h,i,l)}},n=a.extend({},e.datepickerOptions,{onChangeMonthYear:function(h,i,l){j(h,i,l);typeof e.datepickerOptions.onChangeMonthYear==="function"&&m(h,i,l)},beforeShow:function(h,i){datepickerShowed=true;typeof e.datepickerOptions.beforeShow==="function"&&e.datepickerOptions.beforeShow.apply(this,[h,i]);setTimeout(function(){m(i.drawYear,
i.drawMonth+1,i)},1E3)}});d.datepicker(n);setTimeout(function(){var h=d.datepicker("getDate");m(h.getFullYear(),h.getMonth()+1,d)},2E3);return d},built:function(){var b=this;a.fn.EventCalendar=function(c,d){return b.fn.apply(this,[c,d])};return true}})})(jQuery);
...
g+'">'+g.padLeft("0",2)+"</option>");var j=a('<select class="sparkle-time-minutes" />');for(f=0;f<=55;f+=5)j.append('<option value="'+f+'">'+f.padLeft("0",2)+"</option>");var k=a('<select class="sparkle-time-meridian" />');k.append("<option>am</option>");k.append("<option>pm</option>");g=d.val();var m=new Date;f="12";var n="0",h="am";if(g){m.setTimestr(g);f=m.getUTCHours();n=m.getUTCMinutes();if(f>12){f-=12;h="pm"}}k.insertAfter(d);j.insertAfter(d);e.insertAfter(d);if(f>12&&h=="pm")f-=12;e.val(f);
j.val(n.roundTo(5));k.val(h);f=function(){var i=parseInt(e.val(),10),l=j.val();if(k.val()=="pm")i+=12;if(i>=24)i=0;i=i.padLeft(0,2)+":"+l.padLeft(0,2)+":00";d.val(i).trigger("change")};e.add(j).add(k).change(f);d.parent("form:first").submit(f);return d})},built:function(){var b=this;a.fn.timepicker=function(c,d){return b.fn.apply(this,[c,d])};return true}})})(jQuery);
